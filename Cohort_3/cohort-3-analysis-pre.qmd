...
analyze_likert_question <- function(question, design) {
  ...
  # Calculate the weighted table of frequencies
...
# Filter the data for Detroit stratum
detroit_data <- subset(cohort3BackgroundGov, city_mapped == "Detroit")

# Initialize a list to store stats for each question
detroit_stats <- list()

# Loop through each question and calculate stats
for (question in questions_to_analyze) {
    question_data <- detroit_data[[question]]
    # Check if the data is not all NA
    if (!all(is.na(question_data))) {
        ...
        # If data is all NA, set values to NA
        median_val <- NA
        mean_val <- NA
        sd_val <- NA
    }
    # Add the stats to the list
    detroit_stats[[question]] <- list(median = median_val, mean = mean_val, sd = sd_val)
}

# Print stats for each question
for (question in questions_to_analyze) {
    cat("Statistics for", question, ":\n")
    print(detroit_stats[[question]])
    cat("\n")
}

# All other strata
# Function to calculate median and standard deviation for each stratum and question
analyze_likert_stats_stratum <- function(question, data, stratum, design) {
  # Filter data for the specific stratum
  stratum_data <- subset(data, city_mapped == stratum)

  # If stratum is not Detroit, use survey package functions
  if (stratum != "Detroit") {
    median <- svyquantile(~get(question), subset(design, city_mapped == stratum), 0.5, na.rm = TRUE)
    sd <- svysd(~get(question), subset(design, city_mapped == stratum), na.rm = TRUE)
  } else {
    # Direct calculation for Detroit
    if (!all(is.na(stratum_data[[question]]))) {
      median <- median(stratum_data[[question]], na.rm = TRUE)
      sd <- sd(stratum_data[[question]], na.rm = TRUE)
    } else {
      median <- NA
      sd <- NA
    }
...
for (stratum in strata) {
  stratum_stats <- lapply(questions_to_analyze, function(q) analyze_likert_stats_stratum(q, cohort3BackgroundGov, stratum, design))
  all_strata_stats[[stratum]] <- stratum_stats
}

# Example: Print stats for question "q8" for all strata
for (stratum in strata) {
    cat("Statistics for q8 in", stratum, ":\n")
    print(all_strata_stats[[stratum]][["q8"]])
    cat("\n")
}
```
...
ggsave("q23_pre_frequency_plot.png", q23_pre_frequency_plot, width = 11, height = 8.5, dpi = 300)
